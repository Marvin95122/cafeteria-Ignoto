...
<div class="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
    <div>
        ...
        <p class="text-stone-500 mt-1">Gestiona el acceso, roles y estado de tu personal.</p>
    </div>
...
<div class="bg-white p-4 rounded-2xl shadow-sm border border-stone-100 mb-8">
    <form method="GET" action="{{ route('employees.index') }}" class="flex flex-col md:flex-row gap-4 items-center justify-between">
        <div>
            <p class="text-sm font-bold text-stone-700">Filtrar empleados</p>
            <p class="text-xs text-stone-400">Consulta usuarios por estado o rol.</p>
        </div>

        <div class="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
            <select name="status"
                    onchange="this.form.submit()"
                    class="w-full sm:w-56 rounded-xl border-stone-200 bg-stone-50 text-stone-700 shadow-sm focus:border-amber-500 focus:ring focus:ring-amber-200 focus:ring-opacity-50 transition">
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-stone-100 p-6 flex items-start gap-4 hover:shadow-md transition relative group
            ...
            <div class="flex-1">
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    @else
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-stone-200 text-stone-600 border border-stone-300">
                            Inactivo
                        </span>
                    @endif
                </div>
    ...
    @empty
        <div class="col-span-3 text-center py-12 bg-white rounded-xl border border-dashed border-stone-300">
            <p class="text-stone-500">No hay empleados registrados con los filtros seleccionados.</p>
        </div>
    @endforelse
</div>
