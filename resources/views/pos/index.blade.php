...
<script>
    ...
    async function processCheckout() {
        ...
        try {
            // Obtenemos el token de seguridad de Laravel
            ...
            if (data.success) {
                // Generar URL del ticket pasando el efectivo y el cambio
                const ticketUrl = `/pos/ticket/${data.order_id}?received=${payload.cash_received || payload.total}&change=${payload.change || 0}`;
                
                // Abrir el ticket en una ventana emergente pequeña
                window.open(ticketUrl, 'Ticket', 'width=400,height=600');
                
                // Limpiar carrito y recargar la página de ventas después de 1 segundo
                setTimeout(() => {
                    cart = [];
                    document.getElementById('cash-received').value = '';
